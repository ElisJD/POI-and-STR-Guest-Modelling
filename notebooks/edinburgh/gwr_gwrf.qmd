...
edinburgh_airbnb_tourism_fs <- st_read('../../data/edinburgh/edinburgh_airbnb_tourism_fs.gpkg')
```
...
gdf_centroids <- st_centroid(edinburgh_airbnb_tourism_fs)

# Extract coordinates
coords <- st_coordinates(gdf_centroids)

# Add to data frame
edinburgh_airbnb_tourism_fs$X <- coords[, 1]
edinburgh_airbnb_tourism_fs$Y <- coords[, 2]
...
```{r}
bw <- grf.bw(grf_formula, df_clean, coords = coords_clean, kernel = "adaptive", bw.min = 5, bw.max = 48, step = 5, trees = 500, importance = 'impurity', geo.weighted = TRUE)
```
